...
(function(){
  ...
  function applyLang(){
    ...
    document.getElementById('ch-ainote-short').textContent=d.ainoteShort||'';document.getElementById('ch-ainote-full').innerHTML=d.ainote;
  ...
  async function sendToAgent(text){
    ...
    chatBusy=true;
    var _sg=document.querySelector('.ch-sugs');if(_sg)_sg.classList.add('hide');
    appendMsg(text,'usr');
...
/* ── Traslitterazione singalese (Singlish → සිංහල) ──
     Con una tastiera italiana non è possibile digitare caratteri sinhala.
     Quando la lingua è "si" l'utente scrive in latino e vede il sinhala.
     Le sequenze più lunghe vanno provate per prime. */
  var SI_CONS={'kh':'ඛ','gh':'ඝ','ch':'ච','chh':'ඡ','jh':'ඣ','th':'ත','thh':'ථ','dh':'ධ','dd':'ඩ','ph':'ඵ','bh':'භ','sh':'ශ','ss':'ෂ','ng':'ං','nd':'ඳ','mb':'ඹ','nn':'ණ','ll':'ළ','gn':'ඤ',
    'k':'ක','g':'ග','j':'ජ','t':'ට','d':'ද','n':'න','p':'ප','b':'බ','m':'ම','y':'ය','r':'ර','l':'ල','v':'ව','w':'ව','s':'ස','h':'හ','f':'ෆ','z':'ස','c':'ච','q':'ක','x':'ක්ස'};
  var SI_VOW={'aa':'ා','ae':'ැ','aae':'ෑ','ii':'ී','ee':'ී','uu':'ූ','oo':'ෝ','ei':'ෙ','ai':'ෛ','au':'ෞ','a':'','i':'ි','u':'ු','e':'ෙ','o':'ො'};
  var SI_IND={'aa':'ආ','ae':'ඇ','aae':'ඈ','ii':'ඊ','ee':'ඊ','uu':'ඌ','oo':'ඕ','ai':'ඓ','au':'ඖ','a':'අ','i':'ඉ','u':'උ','e':'එ','o':'ඔ'};
  function siKeys(o){return Object.keys(o).sort(function(a,b){return b.length-a.length;});}
  var SI_CK=siKeys(SI_CONS),SI_VK=siKeys(SI_VOW),SI_IK=siKeys(SI_IND);
  function toSinhala(txt){
    var out='',i=0,low=txt.toLowerCase();
    while(i<txt.length){
      var ch=low[i];
      if(!/[a-z]/.test(ch)){out+=txt[i];i++;continue;}
      var cons=null;
      for(var a=0;a<SI_CK.length;a++){var k=SI_CK[a];if(low.substr(i,k.length)===k){cons=k;break;}}
      if(cons){
        i+=cons.length;
        var vow=null;
        for(var b=0;b<SI_VK.length;b++){var v=SI_VK[b];if(v&&low.substr(i,v.length)===v){vow=v;break;}}
        if(vow===null&&low.substr(i,1)==='a'){vow='a';}
        if(vow!==null){out+=SI_CONS[cons]+SI_VOW[vow];i+=vow.length;}
        else{out+=SI_CONS[cons]+'්';}                 // consonante nuda: virama
        continue;
      }
      var ind=null;
      for(var c=0;c<SI_IK.length;c++){var w=SI_IK[c];if(low.substr(i,w.length)===w){ind=w;break;}}
      if(ind){out+=SI_IND[ind];i+=ind.length;continue;}
      out+=txt[i];i++;
    }
    return out;
  }
